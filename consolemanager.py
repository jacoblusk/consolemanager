...
_FillConsoleOutputCharacterW.argtypes = [ctypes.c_void_p, ctypes.c_wchar, ctypes.c_ulong, _COORD, ctypes.POINTER(ctypes.c_ulong)]
...
_ReadConsoleOutputCharacterW.argtypes = [ctypes.c_void_p, ctypes.c_wchar_p, ctypes.c_uint32, _COORD, ctypes.POINTER(ctypes.c_uint32)]
...
_WriteConsoleOutputW.argtypes = [ctypes.c_void_p, ctypes.POINTER(_CHAR_INFO), _COORD, _COORD, ctypes.POINTER(_SMALL_RECT)]
...
class Console:
    ...
    def read_console_line(self, y: int):
        ...
        _ReadConsoleOutputCharacterW(self.handle, ctypes.cast(ctypes.byref(char_buffer), ctypes.c_wchar_p), length, _COORD(0, y), ctypes.byref(chars_read))
    ...
    def read_console_line_attributes(self, y: int):
        ...
        _ReadConsoleOutputW(self.handle, ctypes.cast(char_info_buffer, ctypes.POINTER(_CHAR_INFO)), buffer_size, buffer_coord, ctypes.byref(rectangle))
        char_buffer = ctypes.create_unicode_buffer(buffer_size.X * buffer_size.Y + 1)
    ...
    def write_console_line_attributes(self, x: int, y: int, char_info_buffer, x_end=0):
        ...
        _WriteConsoleOutputW(self.handle, char_info_buffer, buffer_size, buffer_coord, ctypes.byref(rectangle))
...
def scroll_text_up(console, rectangle: Rectangle, clear_rows):
    ...
    for row in range(rectangle.top + 1, ci.window_rectangle.bottom + 1 - rectangle.bottom - clear_rows):
        for clear_row in range(clear_rows):
            ...
            line, attributes, char_info = console.read_console_line_attributes(row + clear_row)
            """
...
if __name__ == "__main__":
    with ConsoleManager() as console:
        ...
        while True:
            ...
            scroll_text_up(console, padding_rectangle, 1)
